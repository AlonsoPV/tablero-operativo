-- =============================================================================
-- Duplica UAT-04, UAT-05 y UAT-06 para 6 responsables.
--
-- Origenes:
--   7845aa86-f3ff-4d63-95b1-9572125cc1ba
--   0fc6cef4-2035-4c27-9522-53d912cd4928
--   fb4e48bd-6f21-4a24-9e65-eac34f624da7
--
-- Responsables destino (usuarios.id o auth.users.id):
...
-- 0) Diagnóstico de responsables destino
SELECT
  ref.user_ref,
  u.id AS usuarios_id,
...
FROM (
  VALUES
    ('3d0eec63-3feb-4a3d-976b-997255896073'::uuid),
    ('2e0af8cc-03a6-445a-ae1e-d4e9e6f7b3ae'::uuid),
    ('e23561a2-20cc-40cb-9dad-6de520d1ec28'::uuid),
    ('0af758ca-779f-44d1-815c-a8916917cf67'::uuid),
    ('f3b57fa8-9d7d-42f1-8da0-4d8c22ea420e'::uuid),
    ('5700cc85-100e-4c62-89ad-34ce54eb0c8c'::uuid)
) AS ref(user_ref)
...
WHERE id IN (
  '7845aa86-f3ff-4d63-95b1-9572125cc1ba'::uuid,
  '0fc6cef4-2035-4c27-9522-53d912cd4928'::uuid,
  'fb4e48bd-6f21-4a24-9e65-eac34f624da7'::uuid
)
  ...
  FOREACH v_user_ref IN ARRAY v_user_refs LOOP
    ...
    FOREACH v_source_id IN ARRAY v_source_ids LOOP
      ...
      IF v_existing_id IS NOT NULL THEN
        RAISE NOTICE 'Ya existe copia de "%" para usuarios.id % → acción %', v_source.titulo_accion, v_resp, v_existing_id;
      ...
      RAISE NOTICE 'Copia creada: "%" → % (usuarios.id %, ref %)', v_source.titulo_accion, v_new_id, v_resp, v_user_ref;
...
WHERE a.titulo_accion IN (
  SELECT src.titulo_accion
  FROM public.acciones_diarias src
  WHERE src.id IN (
    '7845aa86-f3ff-4d63-95b1-9572125cc1ba'::uuid,
    '0fc6cef4-2035-4c27-9522-53d912cd4928'::uuid,
    'fb4e48bd-6f21-4a24-9e65-eac34f624da7'::uuid
...
ORDER BY u.nombre, a.titulo_accion;
