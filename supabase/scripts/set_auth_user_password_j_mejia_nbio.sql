...
-- Usuario: j.mejia@nbio.mx
-- Contraseña: emx@2026
--
-- Ejecutar en: Supabase Dashboard → SQL Editor (proyecto prod: xhpasmjzuwifmjhrsumb).
--
-- Alternativa (sin SQL), desde la raíz del repo con .env cargado:
--   npm run auth:password -- j.mejia@nbio.mx "emx@2026"
-- =============================================================================
...
WHERE lower(trim(au.email)) = lower(trim('j.mejia@nbio.mx'));

DO $$
DECLARE
  v_email constant text := 'j.mejia@nbio.mx';
...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    RAISE EXCEPTION
      'No existe fila en auth.users con email %. Crea el usuario en Authentication → Users antes de ejecutar este script.',
      v_email;
  END IF;
