-- Allow actions to be marked as Hecho without uploaded evidence.
-- Permissions and pending-checkpoint validation remain unchanged.
...
COMMENT ON FUNCTION public.try_set_accion_hecho(uuid, uuid) IS
  'Cierra una accion en una transaccion: permisos + checklist completo; la evidencia no bloquea.';
