...
Deno.serve(async (req) => {
  ...
  // Insertar en distance_catalog (unicidad por origin_id, destination_id, route_mode)
  const insertPayload = {
    origin_id: originId,
  ...
  if (insertError) {
    if (insertError.code === '23505') {
      ...
      if (existing) {
        return json({
          ok: true,
          km_ida: Number(existing.km_ida),
          km_vuelta: Number(existing.km_vuelta),
          km_total: Number(existing.km_total),
          distance_catalog_id: existing.id,
          duracion_ida_segundos: existing.duracion_ida_segundos ?? undefined,
          duracion_vuelta_segundos: existing.duracion_vuelta_segundos ?? undefined,
          cached: true,
        })
      }
    }
  ...
  return json({
    ...
    duracion_ida_segundos: ida.duracionSegundos ?? undefined,
    duracion_vuelta_segundos: vuelta.duracionSegundos ?? undefined,
    cached: false,
  })
})
