...
describe('accionUtils fecha compromiso CDMX', () => {
  ...
  it('marca retraso solo si la fecha compromiso ya paso (no por hora del mismo dia)', () => {
  ...
  it('promueve a Hoy las acciones del mismo dia aunque la hora limite ya paso', () => {
    expect(getAutoEstadoPorFechaCompromiso(baseAccion({ estado: 'Pendiente' }))).toBe('Hoy')
    expect(getAutoEstadoPorFechaCompromiso(baseAccion({ estado: 'Hoy' }))).toBeNull()
  })

  it('conserva En_Ejecucion cuando la fecha compromiso es hoy', () => {
    expect(getAutoEstadoPorFechaCompromiso(baseAccion({ estado: 'En_Ejecucion' }))).toBeNull()
    expect(getAccionKanbanColumn(baseAccion({ estado: 'En_Ejecucion' }))).toBe('En_Ejecucion')
  })
  ...
  it('ubica en kanban segun fecha compromiso', () => {
