/**
 * Formulario del tablero de distancias: ruta, fecha, hora, origen/destino (dropdown + ubicación readonly),
 * botón Calcular distancia y botón Guardar solicitud.
 */
...
export function DistanceRequestForm({
  ...
  const canCalculate = !!originId && !!destinationId && !isCalculatePending
  const canSave =
    !!lastResult?.ok &&
  ...
  const handleCalculate = () => {
    ...
    form.clearErrors()
    onCalculate(originId, destinationId)
  }

  const handleSave = form.handleSubmit((values) => {
    if (!lastResult?.ok || lastResult.km_ida == null || lastResult.km_vuelta == null || lastResult.km_total == null)
      return
    ...
    >
      <Card id="distance-request-card" className="distance-request-card border-border/60 bg-muted/5">
        ...
        <CardContent id="distance-request-card-content" className="distance-request-card-content space-y-4">
          {/* Bloque: identificación de la ruta */}
          ...
          <div id="distance-request-actions" className="distance-request-actions flex flex-wrap gap-2">
            ...
            >
              {isCalculatePending ? 'Calculando…' : 'Calcular distancia'}
            </Button>
            <Button
            ...
            >
              {isSavePending ? 'Guardando…' : 'Guardar solicitud'}
            </Button>
          </div>
        </CardContent>
      </Card>
