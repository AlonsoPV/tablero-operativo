-- =============================================================================
-- Aplicación manual del reemplazo KPI core (docs/KPIs.md) en bases ya desplegadas.
--
-- Uso típico:
--   psql $DATABASE_URL -f scripts/apply-kpis-md-baseline.sql
--   (o pegar en el SQL editor de Supabase)
--
-- Prerrequisitos:
--   - Existe el seed O2C de 10 KPIs (nombres "O2C — OTIF", …) aún sin migrar.
--   - Si ya ejecutaste supabase/migrations/20260415120000_replace_catalog_kpis_with_kpis_md_core.sql,
--     este script no hará cambios (idempotente: detecta KPI-01 —).
--
-- Advertencia: borra mediciones históricas de los 10 catalog_kpi_id afectados y pone current_value NULL.
--
-- Contenido: equivalente a la migración homónima; mantener sincronizado si se ajustan valores.
-- =============================================================================
...
BEGIN
  IF EXISTS (SELECT 1 FROM public.catalog_kpis WHERE nombre LIKE 'KPI-01 —%' LIMIT 1) THEN
    RAISE NOTICE 'apply-kpis-md-baseline: ya aplicado (KPI-01 —). Skip.';
    RETURN;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM public.catalog_kpis WHERE nombre = 'O2C — OTIF' LIMIT 1) THEN
    RAISE NOTICE 'apply-kpis-md-baseline: no se encontró O2C — OTIF. Skip.';
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'apply-kpis-md-baseline: completado.';
END $$;
